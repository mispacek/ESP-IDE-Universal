...
class PCF8574:
    ...
    def check(self):
        if self._i2c.scan().count(self._address) == 0:
            raise OSError(f"PCF8574 nenalezen na I2C addrese {self._address:#x}")
        return True
    ...
    def _read(self):
        try:
            ...
            print("Chyba cteni dat z PCF8574 : ", err_code_str)

    def _write(self):
        try:
            self._i2c.writeto(self._address, self._port)
        except Exception as err_code_str:
            print("Chyba zapisu dat do PCF8574 : ", err_code_str)
