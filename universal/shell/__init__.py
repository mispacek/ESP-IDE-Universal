...
def ifconfig():
    ...
    try:
        print('IP address:', terminal_color(_wc.ifconfig()[0]))
        print('subnet mask:', _wc.ifconfig()[1])
        print('gateway:', _wc.ifconfig()[2])
        print('DNS server:', _wc.ifconfig()[3])
    except Exception as e:
        print("Exception: {0}".format(e))

    from ubinascii import hexlify
    try:
        MAC = terminal_color(hexlify(_wc.config('mac'), ':').decode())
    except:
        MAC = "Err: w.sta_if"
...
def wifi(comm="on"):
    ...
    if comm == "scan":
        ...
        print("networks:")
        print('-' * SEPARATOR_WIDTH)
        nets = [[item[0].decode('utf-8'), hexlify(item[1], ":").decode(), item[2], item[3], item[4]] for item in _wc.scan()]
        for net in nets:
            print(str(net))
        print('-' * SEPARATOR_WIDTH)
        _wc.active(staactive)
